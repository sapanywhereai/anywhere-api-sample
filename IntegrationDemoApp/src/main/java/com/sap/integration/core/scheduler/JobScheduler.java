...
public class JobScheduler {
    ...
    public static void run() {
        ...
        try {
            JobDetail accessTokenJob = JobBuilder.newJob(AccessTokenJob.class).build();
            JobDetail integrationJob = JobBuilder.newJob(IntegrationJob.class).build();

            Trigger accessTokenTrigger = TriggerBuilder.newTrigger()
                    .withSchedule(CronScheduleBuilder.cronSchedule(Property.getSchedulerAccessToken())).build();
            ...
            scheduler.start();
            scheduler.scheduleJob(accessTokenJob, accessTokenTrigger);
